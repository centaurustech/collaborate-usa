...
function getAbsoluteCellIndex(oTable,oTR,oTD)//base 1
    ...
    for(var i=0;i<oTR.childNodes.length;i++)
        ...
        if(bFinish==false)
            {
            nCount+=getSpanNum(oTR.childNodes[i].getAttribute("COLSPAN"));
            }
...
function getNextRowLayout(oTable,oTR,oTD)
    ...
    for(var k=0;k<rSpan;k++) oTR_tmp=oTR_tmp.nextSibling;
...
function spanCol()
    ...
    if(oTD.nextSibling)
        {
        if(getSpanNum(oTD.getAttribute("ROWSPAN"))==getSpanNum(oTD.nextSibling.getAttribute("ROWSPAN")))
            {
            oTD.setAttribute("COLSPAN", getSpanNum(oTD.getAttribute("COLSPAN")) + getSpanNum(oTD.nextSibling.getAttribute("COLSPAN")));
...
function splitCol()
    ...
    if(oTD.nextSibling)
        {
        if(getSpanNum(oTD.getAttribute("ROWSPAN"))!=getSpanNum(oTD.nextSibling.getAttribute("ROWSPAN")))return;
        }
    //~~~~ NEW ~~~~~~
