...
function InnovaEditorUtil()
    ...
    for(var i=0;i<oScripts.length;i++)
    {
        ...
        if(sSrc.indexOf("moz/editor.js")!=-1)
        {
            ...
            //this.scriptPath=oScripts[i].src.replace(/moz\/editor.js/,"");
        }
...
function InnovaEditor(oName)
    ...
    for(var i=0;i<oScripts.length;i++)
    {
        ...
        {
            this.scriptPath=oScripts[i].src.replace(/editor.js/,"")
            //this.scriptPath=this.scriptPath.replace(/moz\//,"");
            //alert(this.scriptPath);
        }
    }
...
function saveForUndo()
    ...
    obj.focus();
    var oSel=oEditor.getSelection();
    if(oSel.rangeCount<=0){return false;}
...
function RENDER(sPreloadHTML)
    ...
    for(var i=0;i<this.buttonMap.length;i++)
    {
        var sxe_31 = this.scriptPath;
        //sxe_31 = sxe_31.replace(/moz\//,"");
        ...
        switch(sButtonName)
        {
            ...
            case "Bookmark":
                ...
                break;
            case "Hyperlink":
                if(this.btnHyperlink)sHTMLIcons+=this.writeIconStandard("btnHyperlink"+this.oName,"modelessDialogShow('"+sxe_31+"hyperlink.htm',420,150)","btnHyperlink.gif",getText("Hyperlink"));
                break;
            case "CustomTag":
                ...
                break;
            case "Image":
                if(this.btnImage)sHTMLIcons+=this.writeIconStandard("btnImage"+this.oName,"modelessDialogShow('"+sxe_31+"image.htm',440,305)","btnImage.gif",getText("Image"));
                break;
            case "Flash":
            ...
            case "Table":
                if(this.btnTable)
                    ...
                    for(var m=0;m<8;m++)
                        ...
                        for(var n=0;n<8;n++)
                            {
                            sHTMLDropMenus+="<td onclick='"+this.oName+".doClick_TabCreate(this)' onmouseover='doOver_TabCreate(this);event.cancelBubble=true;' style='background:#ffffff;font-size:1px;border:#d3d3d3 1px solid !important;width:20px;height:20px;' unselectable=on>&nbsp;</td>";
                            }
                        sHTMLDropMenus+="</tr>";
                        }
