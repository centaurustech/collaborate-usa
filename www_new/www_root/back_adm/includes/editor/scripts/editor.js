...
function InnovaEditor(oName)
	...
	//this.btnHTMLFullSource=false;this.btnHTMLSource=false;this.btnXHTMLFullSource=false;
...
function RENDER(sPreloadHTML)
	...
	sHTML+="<tr><td colspan=2 valign=top height=100% style='background:white; !important'>";
...
function loadHTML(sHTML)//hanya utk first load.
	...
	if(this.arrStyle.length>0)
	{
		...
		for(var i=0; i<this.arrStyle.length; i++)
		{
            ...
            arr2 = selector.split(/,/);
            for(iu=0; iu<arr2.length; iu++)
            oEditor.document.styleSheets(0).addRule(arr2[iu], style);
		}
	}
	//*** RUNTIME STYLES ***
	this.runtimeBorder(false);
...
function doCmd(sCmd,sOption)
{
    ...
    if(sCmd=='fontsize')
    {
        ...
        //oElement.style.fontSize = sOption;
...
function writeIconToggle(id,command,img,title)
	...
	sHTML=""+
		...
		"<img name=\""+id+"\" id=\""+id+"\" btnIndex=\""+buttonArraysCount+"\" unselectable='on' src='"+imgPath+"' style='position:bsolute;top:-0;width:"+w+"px'"+
