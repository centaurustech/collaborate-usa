...
$ignore = array_flip(array('pageindex', 'search_it', 'email_add', 'voice_cat_id', 'question_text', 'vc_dir', 'votes_count')); //for reset type func
...
if(isset($_POST['command'])) //delete
{
    ...
    #/ Get files to delete
    $sq = sprintf("SELECT voice_pic, user_id from voices_dump WHERE original_voice_id IN (%s)", $rid_csv);
    $c_info = mysql_exec($sq);


    #/ Delete Voice
    $query = sprintf("DELETE FROM voices_dump WHERE original_voice_id IN (%s)", $rid_csv);
    mysql_query($query);

    #/ Delete Vote
    $query = sprintf("DELETE FROM voices_votes_dump WHERE voice_id IN (%s)", $rid_csv);
    mysql_query($query);
    ...
    {
        if($v['voice_pic']!=''){
        @unlink("../user_files/prof/{$v['user_id']}/voices/{$v['voice_pic']}");
        }
    }
...
$operation_page = 'voices_dump_opp.php';

/////////////////////////////////////////////////////////////////////////
$load_fancy = true;

$pg_title = "Member Voices (archive)";
...
<div>
	...
	if (in_array($orderby, array(1, 2, 3, 4, 5)) == false) {
	$orderby = 5;
	}
    ...
    FROM voices_dump uv
    LEFT JOIN (SELECT vv.voice_id, COUNT(*) AS cx FROM voices_votes_dump vv GROUP BY vv.voice_id) vv ON vv.voice_id=uv.original_voice_id
    LEFT JOIN users u ON u.id=uv.user_id

    WHERE 1 = 1 %s
    GROUP BY uv.original_voice_id
    HAVING 1=1 %s
	ORDER BY %s %s
    LIMIT %d, %d
    ...
    <tr>
        ...
        <th valign="top" width="36%" nowrap><a href="<?php echo rebuildurl(array("orderby" => 3, "orderdi" => $orderby == 3 && $orderdi == "ASC" ? "DESC" : "ASC")); ?>">
            Voice Question
        ...
        <th valign="top" width="15%" nowrap><a href="<?php echo rebuildurl(array("orderby" => 2, "orderdi" => $orderby == 2 && $orderdi == "ASC" ? "DESC" : "ASC")); ?>">
            Category
        ...
        <th valign="top" width="15%" nowrap><a href="<?php echo rebuildurl(array("orderby" => 5, "orderdi" => $orderby == 5 && $orderdi == "ASC" ? "DESC" : "ASC")); ?>">
            Created On
	...
	else
	{
		...
		while($recrd)
		{
            ...
            <tr style="background:<?php echo $tr_bg; ?>">
                <td valign="top"><input type="checkbox" name="RecordID[]" value="<?php echo $recrd["original_voice_id"]; ?>" /></td>
                ...
                <td valign="top"><?php if($recrd["votes_count"]>0){echo "<a href=\"{$consts['DOC_ROOT_ADMIN']}voice_dump_votes_ajax.php?&vc_id={$recrd["original_voice_id"]}&ro=1\" class=\"fbox fancybox.ajax\" title=\"Votes\">".number_format((int)$recrd["votes_count"], 0)."</a>";}else{echo '0';} ?></td>
