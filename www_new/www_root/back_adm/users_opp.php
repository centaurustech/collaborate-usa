...
if (($u_id) && (empty($empt)) )
{
    //(SELECT COUNT(*) cx FROM streams_voice WHERE user_id=u.id) stream_count,

    $query  = sprintf("SELECT *, u.id AS u_id,

    (SELECT COUNT(*) cx FROM user_voices WHERE user_id=u.id) voices_count,
    (SELECT COUNT(*) cx FROM voices_votes WHERE user_id=u.id) votes_count,
    (SELECT COUNT(*) cx FROM voices_dump WHERE user_id=u.id) purged_voice_count,
    (SELECT COUNT(*) cx FROM eco_members WHERE user_id=u.id) eco_membership_count,
    (SELECT COUNT(*) cx FROM eco_system WHERE moderator_id=u.id) eco_ownership_count,
    (SELECT COUNT(*) cx FROM eco_discussions WHERE posted_by=u.id) eco_discussions_count,
    (SELECT COUNT(*) cx FROM eco_discussion_comments WHERE posted_by=u.id) discussion_comments_count
	...
	$token  = mysql_query($query, $cn1);// or die(mysql_error());
    ...
    if(empty($empt['user_id']))
    $empt['user_id'] = $empt['u_id'];
}
...
<table border="0" cellpadding="0" cellspacing="0" class="datagrid" width="100%">
    ...
    <td valign="middle" style="padding:6px 4px;"><br />
        ...
        if((@$empt['profile_pic']!='') && (@$empt['user_id']!=''))
        {
    ...
    <tr>
    <td valign="middle" style="padding:6px 4px;"><br />
        <div style="width:160px; float:left;">Total Patronage Points:</div>
        <div style="float:left;"><?=number_format((int)format_str(@$empt['total_patronage_points']), 0)?></div>

        <div style="clear:both; height:20px;"></div>


        <div style="width:160px; float:left;">Total Voices (active):</div>
        <div style="float:left;"><?=(int)format_str(@$empt['voices_count'])?></div>

        <div style="clear:both; height:10px;"></div>

        <div style="width:160px; float:left;">Total Voices (purged):</div>
        <div style="float:left;"><?=(int)format_str(@$empt['purged_voice_count'])?></div>

        <div style="clear:both; height:10px;"></div>

        <div style="width:160px; float:left;">Total Votes Given:</div>
        <div style="float:left;"><?=(int)format_str(@$empt['votes_count'])?>
        <span class="submsg">&nbsp;&nbsp;(counting in active Voices only)</span></div>

        <div style="clear:both; height:10px;"></div>

        <div style="width:160px; float:left;">Eco-System Memberships:</div>
        <div style="float:left;"><?=(int)format_str(@$empt['eco_membership_count'])?>
        ...
        <div style="float:left;"><?=(int)format_str(@$empt['eco_ownership_count'])?>
        <span class="submsg">&nbsp;&nbsp;(i.e. moderator count)</span></div>

        <div style="clear:both; height:10px;"></div>

        <div style="width:160px; float:left;">Eco-System Participations:</div>
        <div style="float:left;"><?=(int)format_str(@$empt['discussion_comments_count'] + @$empt['eco_discussions_count'])?>
