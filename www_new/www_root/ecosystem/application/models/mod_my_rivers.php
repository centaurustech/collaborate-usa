...
class Mod_My_Rivers extends Mod_Voice {
    ...
    public function __construct(){
        parent::__construct();
        $this->log("Model Mod_My_Rivers loaded.");
    ...
    public function get_my_rivers($bundle = array()){
        ...
        if($uid){
            ...
            $query = "SELECT * FROM eco_system WHERE `moderator_id`=%d AND `parent_id`=0 AND `level`=2 AND `is_admin_blocked`=0 ORDER BY id DESC LIMIT %d, %d";
            $sql = sprintf($query, $uid, $start, $limit);
            
            $bundle = array("sql" => $sql);
            
            // get voices
            $rivers = $this->get_voices($bundle);
            
            $result["is_more_data"] = $this->is_more_data(sprintf($query, $uid, $next_start, $limit));
            $result["status"]  = $rivers["status"];
            $result["message"] = $rivers["message"];
            $result["is_data"] = $rivers["is_data"];
            $result["data"]    = $rivers["data"];
        }
    ...
    public function mr_to_html($data = array()){
        ...
        if(is_array($data)){
            ...
            if(count($data) > 0){
                ...
                foreach($data as $river){
                    
                    $river_id = $river['id'];
                    $title = strip_tags($river['title']);
                    $title = word_limiter($title, 7);
                    $image = "../user_files/prof/" . $this->get_logged_uid() . "/ecosystem/" . $river['eco_pic'];
                    if(file_exists($image)){$image = "../{$image}";}
                    ...
                    $single_stream_url = base_url() . $this->_config['single_river_url'] . '/' . $river_id;
