...
class Mod_Search extends Mod_Voice {
    ...
    public function __construct(){
        parent::__construct();
        $this->log("Model Mod_Search loaded.");
    ...
    public function get_search_feeds($bundle = array()){
        ...
        $search = c_pick_param($bundle, "search", "");
        ...
        $query = "
                ...
                AND (
                  streams.`question_text`
                  LIKE ('%{$search}%')
                  OR
                  streams.`voice_details`
                  LIKE ('%{$search}%')
                )
        ...
        $query = "
                ...
                ORDER BY created_on DESC                
                LIMIT {$next_start}, {$limit}";
        
        // get feeds
        $feeds = $this->get_voices($bundle);
        
        $result["is_more_data"] = $this->is_more_data($query);
