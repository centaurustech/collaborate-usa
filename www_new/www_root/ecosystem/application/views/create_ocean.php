...
<div class="leftcont brdrnone"> 
    <span class="mainbanner">
        <img src="<?php echo c_get_assets_url(); ?>images/my_oceans.jpg" alt="Create Ocean" />
    </span>
    <div class="container_588 bkgrnd">
        <form  name="ocean_post" id="ocean_post" action="<?php echo base_url() . $config['ocean_create_url']; ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="starheadsev">
                  ...
                  <input name="voc_title" class="leftsinpt wdthnin txt-voc-title" placeholder="Ocean Title"/>                   
                  <textarea name="voc_desc" cols="1" rows="2" class="leftstxtarea wdthnin" placeholder="Ocean Description"></textarea>
        ...
        <input type="hidden" name="oc_key" value="<?php echo $oc_key; ?>" />
        </form>
    </div>
</div>
    ...
    $(document).ready(function(){
        ...
        $('.btn-create-voice').bind('click', function(){
            ...
            if($('.txt-voc-title').val() == ''){
                metroAlert("Ocean Title can't be blank.", {theme: metroStyle.ERROR});
            }
            
            // fild upload must
            else if($('#original').val() == ""){
                metroAlert("Select ocean picture.", {theme: metroStyle.ERROR});
            }
            else{
                if(! lock){
                    lock = true;
