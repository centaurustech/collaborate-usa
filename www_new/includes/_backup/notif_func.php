...
/** Function generate_notification
  ...
  * [params]
  * $notif_data = array containing keys (template_id, user_id, from_user_by, objects, object_id, object_location, visit_url, notif_details)
...
function generate_notification($notif_data, $send_email=false)
{
    ...
    (template_id, user_id, from_user_by, objects, object_id, object_location, visit_url, created_on)
    VALUES ('{$notif_data['template_id']}', '{$notif_data['user_id']}', '{$notif_data['from_user_by']}', '{$notif_data['objects']}', '{$notif_data['object_id']}', '{$notif_data['object_location']}', '{$notif_data['visit_url']}', '{$created_on}')";
    ...
    if($send_email!=false)
    {
        ...
        $from_usr = @$notification_data['users_info'][$notif_data['from_user_by']];
...
function get_notification_msg($notif_data, $send_email=false, $get_user_info=true)
{
    ...
    $from_user_by = $notif_data['from_user_by'];
    ...
    ELSE 'Member'
    END) AS user_ident
    FROM users WHERE id IN ('{$user_id}', '{$from_user_by}')";
    ...
    if(($from_user_by<=0 && count($users_info)<1) || ($from_user_by>0 && count($users_info)<2)){return false;}
    ...
    array(@$users_info[$from_user_by]['user_ident'], @$users_info[$user_id]['user_ident'],  "<b>{$objects_ar[0]}</b>", "<b>{$objects_ar[1]}</b>"),
...
*/
function read_notification($user_id, $notification_id='0')
{
    if($user_id<=0){return false;}

    $sql_notif = "SELECT * FROM user_notifications WHERE user_id='{$user_id}' ";
    if($notification_id>0){$sql_notif.= " AND id='{$notification_id}'";}
    $sql_notif.= " ORDER BY created_on DESC";
    ...
    foreach($notif_res as $notif_v)
    {
        ...
        'from_user_by' => "{$notif_v['from_user_by']}",
        ...
        $notification = @get_notification_msg($notif_data, false, false);
